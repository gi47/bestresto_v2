...
import com.bestresto.springboot.config.auth.dto.OAuthAttributes;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        /* - registrationId:  현재 로그인 진행 중인 서비스를 구분(네이버? 구글? )
        - userNameAttributeName: OAuth2 로그인 진행 시 키가 되는 필드값(Primary Key와 같은 의미)
        - 구글의 경우 기본적으로 코드를 지원("sub"), 네이버 카카오 등은 기본 지원하지 않음.
        */
